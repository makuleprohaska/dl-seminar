...
learning. The same protocol is applied to each encoder for comparison. For BACH and MHIST we report, over
five runs as produced by \texttt{eva}, the average multiclass accuracy (BACH) and binary balanced
accuracy (MHIST); the slide-level CAMELYON16 probe is a single train/test run. In each column the
best result is shown in \textbf{bold} and the second-best is \underline{underlined}.
...
three billion-parameter forward passes with one. The student stays competitive across all three
tasks: it ranks second on MHIST and on the slide-level CAMELYON16 task (behind Virchow2 in both),
and third on BACH. It clearly surpasses the $1.1$\,B-parameter H-optimus-1 on both BACH and MHIST
and beats UNI2-h on MHIST and CAMELYON16, while using $7$--$13\times$ fewer parameters than any
teacher. BACH separates the encoders most strongly here, spanning roughly $78$--$91\%$ and led by
UNI2-h.
...
\midrule
UNI2-h~\cite{uni}        & 681\,M & \textbf{91.46 $\pm$ 0.29} & 82.27 $\pm$ 0.11 & 88.37 \\
Virchow2~\cite{virchow2} & 632\,M & \underline{88.00 $\pm$ 0.76} & \textbf{86.11 $\pm$ 0.05} & \textbf{96.12} \\
H-optimus-1~\cite{hoptimus} & 1.1\,B & 78.07 $\pm$ 0.70 & 83.79 $\pm$ 0.13 & 92.25 \\
\midrule
Student (ours)           & 86.6\,M & 85.16 $\pm$ 0.54 & \underline{84.39 $\pm$ 0.04} & \underline{93.80} \\
\bottomrule
\end{tabular}
\end{center}
